...
function Historique_facturationDate() {
  ...
  //end of states
  const getDataDates = async (e) => {
    e.preventDefault();
    try {
      const url = "http://localhost:8000/facture/betweendates/";
      const dataDates = {
        startDate: mindate,
        endDate: maxdate,
      };
      const waitingDetailes = await Axios.post(url, dataDates);
      setData(waitingDetailes.data);
    } catch (err) {
      console.log(err);
    }
  };
    ...
    <div>
      {msgdeleted ? (
        <motion.div className={styles.handle_msges_parent}>
          <motion.div className={styles.handle_msges_child}
      ...
      <div className={styles.histo_fuct}>
        ...
        <div className={styles.child_histo_fact_dates}>
          <form onSubmit={getDataDates}>
            <div>
              <TextField
                required
                type="date"
...
export default Historique_facturationDate;
