...
function HistoriqueGG() {
  ...
  useEffect(() => {
    const fetchSlimBodyHistorique = async () => {
      const url = "http://localhost:8000/api/historique/giantgel/";
    ...
    return getdata.map((data, i) => {
      return (
        <tr key={i}>
          ...
          <td className={styles.addesb_green}>{data.addedG}</td>
          <td className={styles.addesb_green}>{data.routurnG}</td>
          <td className={styles.minus_red}>{data.minusG}</td>
